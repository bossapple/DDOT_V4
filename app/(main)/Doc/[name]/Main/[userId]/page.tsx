...
function EditDocId({ params }: { params: { userId: string, name: string } }) {
  const GET_USER = gql`
  ...
  const [editDoc, setEditDoc] = useState<EditDocType[]>([]);
  ...
  const handleClose = (
    ...
    setOpenDialog(false);
    setOpenAlert(false);
    router.push(`/Doc/${params.name}/Main`);
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const getData = await graphQLClient.request<{ Userinfo: EditDocType[] }>(
          GET_USERINFO, {
          cid: params.userId
        }
        )
        ...
        let docData = getData.Userinfo

        const formattedData = docData.map((prevDoc) => ({
          ...prevDoc,
          province: provinceData.find((provinceId: any) => provinceId.name_th === prevDoc.province)?.id || null,
          amphoe: amphoeData.find((amphoeId: any) => amphoeId.name_th === prevDoc.amphoe)?.id || null,
          tambon: tambonData.find((tambonId: any) => tambonId.name_th === prevDoc.tambon)?.id || null
        }))

        docData = formattedData
  ...
  const handleUpdateUser = async (): Promise<void> => {
    try {
      const cid = editDoc[0]?.CID;
      const finalProvince = convertProvice(editDoc[0]?.province as number) as string;
      const finalAmphoe = convertAmphure(editDoc[0]?.amphoe as number) as string;
      const finalTambon = convertTambon(editDoc[0]?.tambon as number) as string;
      ...
      const { data, errors } = await updateUser({
        variables: {
          cid: cid,
          edits: {
            Firstname: editDoc[0]?.Firstname,
            Lastname: editDoc[0]?.Lastname,
            Gender: editDoc[0]?.Gender,
            dob: editDoc[0]?.dob,
            telephone: editDoc[0]?.telephone,
            homeAddress: editDoc[0]?.homeAddress,
            email: editDoc[0]?.email,
            tambon: finalTambon,
            amphoe: finalAmphoe,
  ...
  const handleOnChangeProvince = (propertyName: string, value: string | number) => {
    setEditDoc((prevObserver) => {
      const updatedAdmin: EditDocType = { ...prevObserver[0] };
          ...
          <Grid container spacing={2} mt={3}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <FormControl fullWidth>
                ...
                <Select 
                  label="อำเภอ" 
                  value={`${editDoc[0]?.amphoe}` || ""}
                  name='amphoe'
                  onChange={(event) => handleOnChangeProvince(event.target.name, event.target.value)}
                >
                  {amphoes
                    .filter((amphoe) => amphoe.province_id === editDoc[0]?.province)
                    .map(amphoe => (
            ...
            <Grid item xs={12} md={6}>
              ...
              <FormControl fullWidth>
                ...
                <Select 
                  label="ตำบล" 
                  value={`${editDoc[0]?.tambon}` || ""}
                  name='tambon'
                  onChange={(event) => handleOnChangeProvince(event.target.name, event.target.value)}
                >
                  {tambons
                    .filter(tambon => tambon?.amphure_id === editDoc[0]?.amphoe)
                    .map(tambon => (
