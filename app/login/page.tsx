...
function Login() {
  // State variables for user login information and alert status
  ...
  const routerUserRole = (
    userRole: string,
    CID: string,
    updatedRoute: RouteType[]
  ) => {
    switch (userRole) {
      case "ADMIN": {
        const adminRoute = updatedRoute.find(
          (item) => item.label === "หน้าหลัก"
        );
        if (adminRoute && CID) {
      ...
      case "OBSERVER": {
        const observerRoute = updatedRoute.find(
          (item) => item.label === "คนไข้ภายใต้การดูแล"
        );
      ...
      case "HOSPITAL": {
        const hospitalRoute = updatedRoute.find(
          (item) => item.label === "หน้าหลัก"
        );
  ...
  const handleLogin = async () => {
    try {
      ...
      setLoginStatus(true); // Assuming this is defined elsewhere
      Cookies.set("user", JSON.stringify(userLogin)); // Assuming Cookies is imported and set correctly
    ...
    >
      ...
      >
        <Alert severity="error">
          username หรือ password ไม่ถูกต้อง {/* Error message */}
        </Alert>
      </Snackbar>
      {/* Login form */}
      <Grid
